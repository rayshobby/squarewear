...
void putnUSBSerial(char *string, BYTE len) {
    int t = 0;
    ...
    if(USBUSARTIsTxTrfReady()) {
        putUSBUSART(string, len);
        while(!USBUSARTIsTxTrfReady() && t<256) {
            CDCTxService();
            t++;
        }
    }
...
void putrsUSBSerial(const ROM char *string) {
...
BYTE getUSBSerial(char *string, BYTE len) {
    ...
    numBytesRead = getsUSBUSART(string,len);
    return numBytesRead;
