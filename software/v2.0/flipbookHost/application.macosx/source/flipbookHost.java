...
public class flipbookHost extends PApplet {
...
int[][][] colors;      // stores pixel colors of all frames
...
int lcColor = color(255, 0, 0);  // background color:
int rcColor = color(0, 0, 0);    // right click color
int mcColor = color(0, 255, 0);  // middle click color
int tempLCColor = lcColor;
...
public void renderPicker(Rectangle r, int c, String t, int idx) {
...
public void updatePixel(int i, int j, int c){
  int x = '0' + i;
  int y = '0' + j;
  int r = (c >> 16) & 0xFF;
  int g = (c >> 8 ) & 0xFF;
  ...
  if(button == btnConnect){
    if(device == null){
      ...
      if(device != null){
        ...
        nFrames = 1;
        currFrame = 0;
        colors = new int[nBoardX*5][nBoardY*7][nFrames];
        times = new int[1];
        tones = new int[1];
...
public void handleSliderEvents(GValueControl slider,GEvent event){
  if(slider == sliderXBoards || slider == sliderYBoards){
    ...
    PIXELSIZE = Math.min((DEFAULT_PIXELSIZE+DEFAULT_PIXELGAP)/nBoardX-DEFAULT_PIXELGAP,
                         (DEFAULT_PIXELSIZE+DEFAULT_PIXELGAP)/nBoardY-DEFAULT_PIXELGAP);
    int[][][] temp = colors;
    colors = new int[5*nBoardX][7*nBoardY][nFrames];
...
public boolean isOverPixel(int i, int j) {
...
public void saveAnimationToFile(String file){
...
public void loadAnimationFromFile(String file){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "flipbookHost" };
    if (passedArgs != null) {
